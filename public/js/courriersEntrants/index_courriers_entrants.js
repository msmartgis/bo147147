$(document).ready(function () {
    ...
    $("#add_piece_btn").on("click", function () {
        ...
        $(".table-piece tr:last").after(
            ...
            "<td> " +
            '<div class = "form-group"> ' +
            '<select name="modes_receptions[]" class="form-control">' +
            markup_select_option +
            "</select>" +
            "</div>" +
            "</td>" +
            ...
            actueDate +
            '"   name="date_reception_doc_input[]"> ' +
            "</div>" +
            "</td>" +
            "<td>" +
            '<div class="form-group">' +
            '<input type="file" name="documents_ulpoad_input[]" data-max-size="2048" accept="image/png, image/jpeg,application/pdf" class="form-control-file">' +
            "</div>" +
            "</td>" +
            "</tr>"
        );
    ...
    $("#add_accuse_reception_btn").on("click", function () {
        if (item_number_accuse_receptions < 3) {
            $(".table-accuse-reception tr:last").after(
                ...
                '<div class = "form-group"> ' +
                '<div class = "checkbox"> ' +
                '<input type="checkbox" id="row_accuse_reception' +
                item_number_accuse_receptions +
                '"  name="record"> ' +
                '<label for="row_accuse_reception' +
                item_number_accuse_receptions +
                '"></label> ' +
                "</div>" +
                "</div>" +
                ...
                "<td>" +
                '<div class="form-group">' +
                '<input type="file" name="accuse_reception_uploads[]" data-max-size="2048" accept="image/png, image/jpeg,application/pdf" class="form-control-file">' +
                "</div>" +
                "</td>" +
                "</tr>"
            );
