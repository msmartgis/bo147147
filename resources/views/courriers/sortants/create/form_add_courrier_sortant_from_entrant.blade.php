...
<section>
    <div class="row">
        <input type="hidden" name="courrier_entrant_id" value="{{$courrier_entrant->id}}">
        ...
        <div class="col-lg-6 col-md-6 col-sm-12" style="padding-left : 15px">
            ...
            <div class="row">
                <div class="col-lg-6">
                    ...
                    @if ($courrier_entrant->personnePhysique != null )
                    <div class="form-group">
                        <input type="hidden" name="personne_physique_from_entrant_id"
                            value="{{$courrier_entrant->personnePhysique()->first()->id}}">
                        {{Form::text('personne_physqique',$courrier_entrant->personnePhysique()->first()->full_name,['class'=>'form-control','required'=>'required','disabled' => 'disabled'])}}
                    </div>
                    @else
                    <div class="form-group">
                        <input type="hidden" name="personne_morale_from_entrant_id"
                            value="{{$courrier_entrant->personneMorale()->first()->id}}">
                        {{Form::text('personne_morale',$courrier_entrant->personneMorale()->first()->raison_social,['class'=>'form-control','required'=>'required','disabled' => 'disabled'])}}
                    </div>
                    @endif
                </div>

                <div class="col-lg-6">
                    <h5 class="{{__('costum_css.float-right-m')}}">{{__('Mode envoi')}} </h5>
                    <div class="form-group">
...
<!-- Step 2 -->
<h6 class="{{__('costum_css.float-right-m')}}">{{__('Acheminement et traitement')}}</h6>
<section>
